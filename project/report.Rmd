...
```{r}
games$month <- as.Date(games$month)
```
...
```{r}
games %>% 
  as.integer(factor(month, levels = month.name))
```
```{r}
one_day <- games %>% 
  filter(gamename == "Counter-Strike: Global Offensive", year == 2021, month == "February")
```

```{r}
one_day$month <- as.numeric(one_day$month)
```
...
ggplot(g_payday, aes(x = month, y = avg, group = year, color = year)) + geom_line() +geom_point() 
```


```{r}
ggplot(g_payday,aes(x = month,y = avg,group = year, color = as.factor(year))) +
  scale_colour_manual(values=c("#ff0000","#ff7300","blue", "purple", "orange", "yellow", "black", "#64b8d1", "pink", "brown" )) +
