...
```
```{r}
Years_PUBG2 <- as.factor(g_pubg$year)
ggplot(g_pubg,aes(x = number_of_month,y = avg, group = year, color = Years_PUBG2)) +
geom_line(size = 1) +
geom_point()+ 
scale_x_continuous(breaks = seq(1, 12, by = 1),expand=c(0, 1))+ 
geom_dl(aes(label = year), method = list(dl.trans(x = x + 0.1), "last.points", cex = 1)) +
...
ggplot(data = games_2015) +
  geom_line(aes(x = number_of_month, y = avg_month)) +
  scale_x_continuous(breaks = seq(1, 12, by = 1),expand=c(0, 1))
...
> Question 2, Jack:  growth of users overtime!
